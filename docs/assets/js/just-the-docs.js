...
{"0": {
    ...
    "doc": "LittleFS filesystem using SD Card media",
    "title": "Description",
    "content": "File System Operations on the SD Card . | The application opens a file named FILE_TOO_LONG_NAME_EXAMPLE_123.JPG on the SD card | Reads the content of the file and creates a directory named Dir1 | Inside the directory, writes the copied content into another file Dir1/FILE_TOO_LONG_NAME_EXAMPLE_123.JPG | . The image file could be any arbitrary JPEG (image) file chosen by the user and then renamed to** FILE_TOO_LONG_NAME_EXAMPLE_123.JPG**. Note . Since the application creates a directory named Dir1, it is important that the a folder with the same name does not exist on the SD card. If a directory named Dir1 is already present on the SD card, the application will fail. File system layer uses: . SDSPI Driver to communicate to SD Card over SPI interface. ",
    ...
    "doc": "LittleFS filesystem using SD Card media",
    "title": "Prepare the LittleFS image SD card",
    "content": "User can use the tool mklittlefs to create the LittleFS FileSystem image with the JPEG file(FILE_TOO_LONG_NAME_EXAMPLE_123.JPG) and use the tool Etcher to flash the image to the SD card You can follow the README.md in mklittlefs repo to compile and bulid the mklittlefs tools, or use the prebuild tools to create the LittleFS image file for the tests. Steps to use mklittlefs to create the LittleFS image with the JPEG file (FILE_TOO_LONG_NAME_EXAMPLE_123.JPG) . | Download the prebuild mklittlefs tool | On Windows PC, unzip i686-w64-mingw32-mklittlefs-1ee4877.zip (If your PC is using other OS, you can unzip other prebuild tool) | Go to directory i686-w64-mingw32-mklittlefs-1ee4877/mklittlefs/ | create directory src_folder | Copy a JPG file to the path i686-w64-mingw32-mklittlefs-1ee4877/mklittlefs/src_folder and rename to FILE_TOO_LONG_NAME_EXAMPLE_123.JPG | Run command mklittlefs.exe -c src_folder -d 5 -a -b 512 -p 16 -s 1073741824 lfs_image.img | . Steps flash the image to the SD card . | Download the tool Etcher | Insert your SD card and launch Etcher | Select the image (lfs_image.img) | Select the device corresponding to your SD card (Etcher proposes you the devices that are removable to avoid erasing your system disk) | Click on the Flash! button | . ",
    "url": "http://localhost:4000/core_apps_sam_d5x_e5x/apps/fs/sdspi_littlefs/readme.html#prepare-the-littlefs-image-sd-card",
    "relUrl": "/apps/fs/sdspi_littlefs/readme.html#prepare-the-littlefs-image-sd-card"
  },"173": {
    "doc": "LittleFS filesystem using SD Card media",
    "title": "Setting up the hardware",
